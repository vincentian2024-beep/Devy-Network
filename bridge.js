...
export async function handleBridgeCommand(message, command, args) {
  if (![
    "bridgepanel", "wallet", "store", "link", "approve", "credit",
    ...
    if (command === "link") {
      ...
      await sendPrivateCommandResult(message, {
        embeds: [embed(
          ...
          `Your delivery account is now linked to \`${username}\`.\n\nFuture Rank and Keys purchases will be delivered to this username.`
        )]
      });
      return true;
    }
    ...
    if (id.startsWith("lb_review_confirm_") || id.startsWith("lb_review_deny_")) {
      ...
      if (confirm) {
        ...
        await notifyEmbed(interaction.client, result.userId,
          embed("Top-up Confirmed", "Your payment was confirmed and your wallet is ready to use.")
            .addFields(
              { name: "Amount Added", value: php(result.amountCentavos), inline: true },
              { name: "New Balance", value: php(result.balanceCentavos), inline: true },
        ...
        await notifyEmbed(interaction.client, payment.userId,
          embed("Top-up Not Confirmed", "Your top-up could not be confirmed and no wallet credit was added.")
            .addFields(
              { name: "Reference", value: `\`${reference}\``, inline: true },
              { name: "Amount", value: php(payment.amountCentavos), inline: true }
            ));
    ...
    if (id === "lb_topup_submit") {
      ...
      await interaction.reply({
        embeds: [embed(
          ...
          "Your secure payment request is now waiting for confirmation. You do not need to upload a screenshot."
        ).addFields(
          ...
          { name: "Payment Reference", value: `\`${reference}\`` },
          {
            ...
            value: process.env.PAYMENT_INSTRUCTIONS ||
              "Send the exact amount. You will receive a private DM when the review is complete."
          }
        )],
        ephemeral: true
      });
